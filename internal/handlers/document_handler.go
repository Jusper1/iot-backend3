...
func (h *DocumentHandler) FindAll(c *gin.Context) {
...
func (h *DocumentHandler) FindByOrderID(c *gin.Context) {
...
func (h *DocumentHandler) FindBySPJID(c *gin.Context) {
	spjID, err := strconv.ParseUint(c.Param("spj_id"), 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"message": "SPJ ID tidak valid",
		})
		return
	}

	data, err := h.Service.FindBySPJID(uint(spjID))
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
			"message": err.Error(),
